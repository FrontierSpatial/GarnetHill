...
function togglePage(page) {
  //console.log(page)
  if (page=='bike') {
    map.addLayer(layers.basemaps["Bike Trail Map"])
    map.removeLayer(layers.basemaps["Ski Trail Map"])
  }
  else if (page=='ski') {
    map.addLayer(layers.basemaps["Ski Trail Map"])
    map.removeLayer(layers.basemaps["Bike Trail Map"])
  }
  else {
    $("#trees").show();
    $("#invasives").hide();
...
const layers = {
  basemaps: {
    ...
    // }).addTo(map),
    "Bike Trail Map": L.tileLayer.mbTiles("data/bike_map.mbtiles", {
      autoScale: true,
      fitBounds: false,
      updateWhenIdle: false
    }).on("databaseloaded", (e) => {
      //layers.basemaps["Bike Trail Map"].bringToFront();
      //map.removeLayer(layers.basemaps["Ski Trail Map"])
      
    }).addTo(map),
    "Ski Trail Map": L.tileLayer.mbTiles("data/ski_map.mbtiles", {
      autoScale: true,
      fitBounds: true,
      updateWhenIdle: false
    }).on("databaseloaded", (e) => {
      map.setMaxBounds(L.latLngBounds(layers.basemaps["Ski Trail Map"].options.bounds).pad(0));
...
initSqlJs({
  ...
  hideLoader();
  //loadData();
  //layers.basemaps["Bike Trail Map"].addTo(map);
